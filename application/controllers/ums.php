...
class Ums extends CI_Controller {
	...
	function uploadImage(){
		try{
			...
			$ret=array('success' =>1 ,'errorCode'=>0 ,'errorMsg'=>'OK','msg'=>$filename,'filename'=>$imgfile);
		}catch(Exception $e){
	...
	*/
	...
	function myCollect(){
		...
		try{
			...
			// $ret=array('success' =>0 ,'errorCode'=>-1 ,'errorMsg'=>'DB error','rs'=>$content->mid,'total'=>$nums->num);
	...
	function addMemberResume(){
		...
		if (!empty($info)) {
			$this->resume_model->changeMemberResume($content,$info->rid);
		}else{
			$this->resume_model->addMemberResume($content);
		}
